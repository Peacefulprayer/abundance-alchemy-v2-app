...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    apiService.getSoundscapes().then(remoteSounds => {
  ...
  useEffect(() => {
    const handleClick = () => {
      if (settings.soundEffectsOn) playBell();
    };
  ...
  useEffect(() => {
    if (!isLoading && hasInteracted && mode === AppMode.SPLASH) {
          ...
          {!hideNav && (
            <nav className={`h-20 backdrop-blur-md border-t absolute bottom-0 w-full z-50 ${settings.theme === 'light' ? 'bg-white/80 border-slate-200' : 'bg-slate-900/60 border-slate-700/50'}`}>
              <div className="flex justify-around items-center h-full max-w-md mx-auto px-4 pb-4">
                <button onClick={() => setMode(AppMode.DASHBOARD)} className={`flex flex-col items-center space-y-1 transition-colors ${getNavClass(AppMode.DASHBOARD)}`}>
                  <Home size={24} />
